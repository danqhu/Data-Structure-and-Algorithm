...
public:
	...
	bool operator== (const CheckedOutBook& cob) const {
		return strcmp(author->name, cob.author->name) == 0 && strcmp(book->title, cob.book->title) == 0;
	}
...
ostream& Patron::printPatron(ostream& out) const{
	...
	if(!books.empty()){
		...
		list<CheckedOutBook>::const_iterator cob = books.begin();
		for(; cob != books.end(); ++cob)
			cout << " * " << cob->author->name << ", " << cob->book->title << endl;
	}
